...
// #Relatório PDF com o valor total dos pagamentos por região em ordem alfabética;
        try {
            $statement = $pdo->prepare("SELECT p.db_value as valor, r.str_name_region as region FROM db_eca.tb_payments as p, db_eca.tb_region as r group by  r.str_name_region order by r.str_name_regions");
            if ($statement->execute()) {
                if ($statement->rowCount() > 0) {
                    $lista = array();
                    while ($x = $statement->fetch(PDO::FETCH_OBJ)) {
